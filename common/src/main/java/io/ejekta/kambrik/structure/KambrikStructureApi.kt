...
class KambrikStructureApi internal constructor() {
    ...
    fun addToStructurePool(server: MinecraftServer, nbtLocation: Identifier, poolLocation: Identifier, processorLocation: Identifier, weight: Int = 10_000) {
        ...
        val emptyProcessorList: Holder.Reference<StructureProcessorList> =
            server.registryAccess().lookupOrThrow(Registries.PROCESSOR_LIST).getOrThrow(EMPTY_PROCESSOR_LIST_KEY)
        ...
        val ourProcessorList: Holder.Reference<StructureProcessorList> =
            server.registryAccess().lookupOrThrow(Registries.PROCESSOR_LIST)
                .get(OUR_PROCESSOR_LIST_KEY).getOrNull() ?: emptyProcessorList

        val poolGrabber = server.registryAccess().lookupOrThrow(Registries.TEMPLATE_POOL).getOptional(poolLocation)
