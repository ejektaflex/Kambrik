...
open class RegistryObjectSerializer<T : Any>(private val reg: () -> Registry<T>, serialName: String) : KSerializer<T> {
    override val descriptor: SerialDescriptor = PrimitiveSerialDescriptor(serialName, PrimitiveKind.STRING)

    override fun serialize(encoder: Encoder, value: T) {
        encoder.encodeString(reg().getKey(value).toString())
    }

    override fun deserialize(decoder: Decoder): T {
        val id = decoder.decodeString()
        return reg().getOptional(Identifier.parse(id)).orElseThrow { SerializationException("Could not find saved identifier!: $id") }
    }
